...
std::optional<xla::BackendKind> GetModuleBackendKind(mlir::Operation* op) {
  ...
  for (auto fn : module.getOps<mlir::func::FuncOp>()) {
    if (auto kind = GetBackendKind(fn)) return kind;
  }
  return std::nullopt;
}
...
struct XlaInlinerInterface : public mlir::DialectInlinerInterface {
  ...
  bool isLegalToInline(mlir::Operation* call, mlir::Operation* callable,
    ...
    if (!wouldBeCloned || contains_call_to_same_function) return true;
    ...
    // CPU workloads.
    return GetNumOps(callable_region->front()) <=
               kMaxMultiCallerInlineOpsCpu &&
           GetModuleBackendKind(call) == xla::BackendKind::kCpu;
  }
