...
TEST_F(CpuCompilerInternalsTest, DotRootedLoopFusionRoutesToMlirEmitter) {
  ...
  // Forming them is fine (the fusion avoids materializing the larger operand);
  // the MLIR loop emitter emits the elemental dot.
  ...
  EXPECT_TRUE(
      ThunkEmitter::FusionRoutesToMlirEmitter(hlo_module->config(), fusion));
}
