...
**Audience:** XLA:CPU contributors and reviewers of the codegen roadmap. The document assumes familiarity with HLO (XLA's high-level IR), MLIR, and the general structure of the XLA:CPU backend. The glossary ([section 8](#8-glossary)) defines the terms this document uses in a specific sense.
...
Numbers labeled "prototype" come from a working implementation of the [M0](#m0-generalize-hoisting-from-whiles-to-runs) passes (Apple M3, single thread, bitwise-identical outputs unless noted).
...
Measurement keeps this story honest in both directions. The census in [section 5.1](#51-legacy-path-census-on-the-local-corpus) found its sharpest single number on the modern path, not the legacy one: a 553x recompute pathology in the current MLIR loop emitter, with zero amplification involved. Every deletion and default flip in this plan is therefore gated on measurement, not on which path is newer.
...
2. **No fixed blockers.** Every op that today cannot join a kernel either gains a modern emission path or an HLO expansion. Two classes remain permanent boundaries: ops that suspend execution (infeed/outfeed, collectives), and opaque runtime services (FFI custom calls, host callbacks, sort, fft). Kernels fold the code around the second class rather than through it; only the optional [M5](#m5-optional-demand-driven-mid-kernel-runtime-calls) would change that.
...
| Autotuning | backend-agnostic framework (`xla/backends/autotuner/`) with production GPU backends (cuDNN/cuBLAS/Triton/native selection, Triton tilings); a CPU implementation exists (`xla/backends/cpu/autotuner/`) | not connected to any decision this doc adds; see [4.5](#45-the-cost-model) for where it fits |
| `SmallWhileLoopHoistingPass` | upstream, default on: outlines a small while (<1 KB accessed) into a call compiled as one kernel | whiles only, 1 KB only; [M0](#m0-generalize-hoisting-from-whiles-to-runs) generalizes it to arbitrary small runs |
| Legacy `IrEmitter` / `IrEmitter2` / `ComputationKernelEmitter` | the backend for hoisted-call kernels and assorted fallbacks | deleted at [M4](#m4-delete-the-legacy-emitters), once every consumer has a modern replacement |
...
Both extremes have measured costs. Recompute of shared chains cost +24% in a hand-edit experiment, and up to 553x where deep unrolled chains turn the shared-producer DAG (directed acyclic graph) into a tree (the MJX kinematics module of [5.1](#51-legacy-path-census-on-the-local-corpus)). Full materialization is nearly free at 32-element sizes (prototype) but writes medium-size intermediates to memory only to reload them immediately.
...
The ordering between per-edge decisions and buffer allocation resolves at HLO time, not emission time. The boundary former ([M0](#m0-generalize-hoisting-from-whiles-to-runs)'s pass, later the cost model) records materialization choices as edge annotations before buffer assignment runs. Any kernel with a scratch-materialized edge takes the **call form**: it is represented in HLO as a kCall to an outlined computation (the same representation hoisted calls use) rather than as a fusion instruction. The distinction matters because buffer assignment treats a fusion body as opaque but recurses into called computations, so it can assign scratch buffers to values inside them. Emission consumes annotations and never invents buffer demand. The pure-fusion syntax therefore needs no new scratch mechanism, and per-edge activation ([M3](#m3-cost-modeled-boundaries-and-large-kernels)) does not wait on the ABI freedom that arrives at [M4](#m4-delete-the-legacy-emitters).
...
The cost model ([4.5](#45-the-cost-model)) makes this a per-site decision. At thunk granularity, library placement wins where the calibrated table says so. Inside a kernel, the op emits natively, or the kernel boundary falls at the op. "Library ops block fusion" stops being an architectural fact and becomes a sizing decision.

Some ops cannot take the native-emission route at all, because they are runtime calls with no HLO decomposition and no emitter. This document calls them the opaque ops. Examples: FFI (foreign function interface) custom calls, such as the LAPACK factorizations inside [jax#33666](https://github.com/jax-ml/jax/issues/33666)'s Newton loop; host callbacks, such as the progress-bar variant of [discussions#24501](https://github.com/jax-ml/jax/discussions/24501); and sort and fft. Two mitigations reduce their cost without any new ABI:

- **Segmentation ([M0](#m0-generalize-hoisting-from-whiles-to-runs)).** The hoisting pass folds the runs of instructions between opaque ops. A loop body that contains one opaque op then pays a handful of dispatches per iteration instead of one per instruction.
- **Small-size HLO expansion.** An opaque op stops being opaque if the compiler can expand it into ordinary HLO, and at the small sizes where folding matters, such expansions are often fast enough. JAX already does this for LU factorization: it carries an HLO-level implementation for platforms where LAPACK is unavailable, and that implementation could serve small sizes everywhere. Sorting networks would do the same for small sorts. Each op handled this way leaves the boundary list without any runtime work.

If a workload still demands folding through an opaque op, the escape hatch is a runtime-context kernel ABI. One pointer to a per-execution context (run options, intra-op threadpool, allocator, FFI execution context, profiler hooks) is added to the kernel calling convention. Call sites resolve at emission time into prebuilt call frames, invoked through a stable C trampoline. The default is a sequential context; a library that manages its own parallelism receives the pool from the context rather than nesting. The pre-thunk emitter's `allow_runtime_calls` mechanism proves feasibility: it is still present in the code, disabled for hoisted-call kernels precisely because the ABI lacks a context. This ABI is scheduled last ([M5](#m5-optional-demand-driven-mid-kernel-runtime-calls)) and built only on demand, because everything else in this roadmap is simpler and delivers its wins sooner. In-kernel copies lower to `llvm.memcpy` regardless, so a folded copy is never slower than the copy thunk it replaces.
...
- Intermediates are placed per [4.1](#41-one-kernel-construct-fusion--hoisted-call-materialization-per-edge).

The driver reuses the existing tile analysis, the kernel ABI's one-result-buffer-per-tuple-leaf mapping, and the bufferization pipeline; the prototype's constant-lifting mechanics carry over.
...
Compile time is a first-class bar because one large function is the known failure mode. LLVM middle-end cost grows ~N^1.8 in straight-line body size (prototype: 2.17 s for a 128-step unrolled chain, against 73 ms at opt-level 0 with ~90% of the runtime win retained). The driver is therefore responsible for limiting how much code lands in one function, and for lowering the optimization level on oversized ones.

The tiling infrastructure underneath is shared property, not a CPU fork. `SymbolicTileAnalysis` and its data structures live in `xla/codegen/tiling/` and serve both the CPU tiled emitter and GPU's Triton compiler (`xla/backends/gpu/codegen/triton/`). GPU is actively expanding the same analysis: in-tree TODOs track removing the single-real-root restriction on multi-output fusions (b/372454662, b/390569102), which is exactly the relaxation item 2 of [4.4](#44-new-emitters-and-capabilities-the-build-list) needs, and broadening op support. Analysis-side coverage work in this roadmap should therefore land in the shared code, where GPU co-maintains it and may close some gaps first. Only the emission side (the driver and the CPU per-op lowerings) is CPU-owned.

### 4.4 New emitters and capabilities (the build list)

Each item below lists the files it touches and the methods it adds or changes. Paths and function names are verified at head as of this writing; line numbers will drift, so the function names are the durable anchors.

1. **Computation driver** ([4.3](#43-the-xtile-computation-driver)): control flow, multi-output, scheduling, materialization. The largest single piece.
   - New `xla/backends/cpu/codegen/tiled/tiled_computation_emitter.{h,cc}`: the driver class, beside the existing tiled fusion emitter. Its entry point takes an outlined `HloComputation` plus the `BufferAssignment`, walks the schedule, and produces one `xtile::EntryFuncOp` module. Control-flow lowering (HLO While to `scf.while`, Conditional to `scf.if`) is new code here; no in-tree emitter lowers HLO control flow to `scf` today, and `elemental_hlo_to_mlir.cc` explicitly lists kWhile and kConditional as unsupported.
   - `xla/backends/cpu/codegen/kernel_api_ir_builder.cc`: no change needed for multi-result kernels. `KernelApiIrBuilder::GetKernelResultsParameters` already emits one result buffer per tuple leaf, which is the ABI mapping the driver needs.
   - `xla/service/cpu/thunk_emitter.cc`: `ThunkEmitter::EmitCallThunk` routes `xla_cpu_small_call` kCalls to the new driver behind the experimental flag; the `ComputationKernelEmitter` path stays until the first [M1](#m1-the-xtile-computation-driver) flip.
   - `xla/service/cpu/parallel_fusion_emitter.cc`: `ParallelFusionEmitter::AddFusion` is the seam for the second flip; it currently calls `EmitFusionKernel` in `xla/backends/cpu/codegen/fusion_emitter.cc`, which tries the tiled emitter and falls back to the loop emitter.
   - `xla/backends/cpu/codegen/tiled/transforms/lower_xtile_entry.cc`: `LowerXTileEntryPass` wraps the entry function in the CPU kernel ABI; it must pass `scf` control-flow ops and multiple results through.
2. **Multi-output tiling:** `SymbolicTileAnalysis` currently accepts multiple roots only when one "real root" consumes the others; hoisted-call shapes have independent roots. Under whole-shape single tiles there is no cross-root consistency problem, so the relaxation is contained.
   - `xla/codegen/tiling/symbolic_tile_analysis.cc`: the restriction is `GetRealRootIndex`, which requires exactly one user-less root and fails with "Only simple multi-output fusions with one real root are supported" otherwise. Relax it to accept independent roots, and teach its callers (`ParameterMappingFromFusionAdaptor` and the root-indexing setup in `AnalyzeComputation`) to carry a root list instead of one index.
   - This is shared code: GPU tracks the same relaxation (b/372454662, b/390569102), so the change lands once for both backends.
3. **Tiled reduce-window:** a masked reduce emission; vectorization then comes free from the existing reduce lowering. Permanently fixes [jax#37465](https://github.com/jax-ml/jax/issues/37465). The windowed indexing map already exists, so the work is concentrated on the emission side.
   - `xla/hlo/analysis/indexing_analysis.cc`: `ComputeOutputToInputReduceWindowOpIndexing` already produces the windowed affine map; extend only if the observed forms need it.
   - `xla/codegen/tiling/symbolic_tile_analysis.cc`: `ShouldProceedWithSymbolicTileDerivation` special-cases only pad, reshape, and bitcast today; add reduce-window if its derivation needs guarding.
   - `xla/codegen/xtile/codegen/fusion_emitter.cc`: `EmitTiledHloInstruction` is the per-op dispatch (an opcode if-chain); add a kReduceWindow case beside the existing kReduce case, emitting the masked reduce and reusing `xla/backends/cpu/codegen/tiled/transforms/vectorized_reduce_emitter.{h,cc}`.
   - `xla/backends/cpu/codegen/tiled/tiled_fusion_emitter.cc`: `IsSupportedInstruction` adds kReduceWindow to the allow-list.
   - Interim fix: extend `ReduceWindowRewriter` in `xla/hlo/transforms/simplifiers/reduce_window_rewriter.{h,cc}` (already in the CPU pipeline in `cpu_compiler.cc`) with the non-overlapping-window to reshape+reduce case, rather than adding a new pass.
4. **Tiled gather, reverse, and dynamic-update-slice:** gather emits per output row as a dynamic-slice of the operand at a runtime index, scoped to observed forms; reverse normalizes the negative stride the analysis already produces; dynamic-update-slice is the write-side dual of the supported dynamic-slice, with the in-place case gated on latency rather than parity, because a naive lowering inserts a full-array self-copy per loop iteration that parity checks cannot see.
   - `xla/codegen/xtile/codegen/fusion_emitter.cc`: add kGather, kReverse, and kDynamicUpdateSlice cases to `EmitTiledHloInstruction`. The existing kDynamicSlice case is the template for both gather (read side, per-row runtime index) and dynamic-update-slice (write side).
   - `xla/hlo/analysis/indexing_analysis.cc`: `ComputeReverseOpIndexing` already exists; reverse is emission-side only, as stated above.
   - `xla/backends/cpu/codegen/tiled/tiled_fusion_emitter.cc`: `IsSupportedInstruction` adds the three opcodes. Today they fall through to the elementwise default and are rejected.
5. **In-kernel sequential scatter (optional):** a loop over index rows under whole-shape tiles; no tiling relation needed. Built only if a workload demands the last measured gap (0.352 vs 0.238 ms on [jax#26145](https://github.com/jax-ml/jax/issues/26145)); small-scatter expansion already covers the class.
   - Same two files as item 4: a kScatter case in `EmitTiledHloInstruction`, and the allow-list entry in `IsSupportedInstruction`.
6. **Mid-kernel runtime-call emission** ([4.2](#42-library-and-runtime-calls-thunks-by-design-native-emission-inside-kernels)): optional, [M5](#m5-optional-demand-driven-mid-kernel-runtime-calls), demand-driven; only for the opaque ops of [4.2](#42-library-and-runtime-calls-thunks-by-design-native-emission-inside-kernels).
   - `xla/backends/cpu/codegen/kernel_api_ir_builder.cc`: the kernel receives a single call-frame argument built by `EmitKernelPrototype`; add the context pointer as a call-frame field, so the function signature itself does not change.
   - `xla/backends/cpu/runtime/kernel_thunk.cc`: `KernelThunk::ExecuteInternal` packs the kernel arguments and launches; thread the context from `ExecuteParams` into the call frame there.
   - New file under `xla/backends/cpu/runtime/`: the trampoline registry keyed by target symbol, and the prebuilt call frames stored with the executable.
7. **Parallel tiles:** tile loops mapped onto the existing kernel workgroup dimensions, so one large kernel uses the intra-op pool. This lets kernel sizes grow without losing parallelism, replacing per-op partitioning for fused work.
   - `xla/backends/cpu/codegen/fusion_emitter.cc`: `GetWorkGroupCount` currently returns the product of `outer_dimension_partitions` (set by `ParallelTaskAssigner`), default 1; derive the workgroup count from the tile count and the cost model instead.
   - `xla/backends/cpu/codegen/tiled/tiled_fusion_emitter.cc`: `CreateTiledKernelDefinition` computes `tiles_per_workgroup` and stamps it into `xtile::TilingInfoAttr`; it takes the new count.
   - `xla/backends/cpu/codegen/tiled/transforms/lower_xtile_entry.cc`: the pass maps `workgroup_id` to a starting tile id; this mapping is where the tile loop meets the pool.
8. **Memcpy-aware copy emission.**
   - The driver (item 1) lowers kCopy members directly to `llvm.memcpy`.
   - `xla/backends/cpu/codegen/tiled/transforms/memref_copy_to_loops.cc`: `MemrefCopyToLoopsPass` currently lowers `memref.copy` to loops; route contiguous copies to memcpy instead.

The [M0](#m0-generalize-hoisting-from-whiles-to-runs) boundary former is a pass rather than an emitter, so it is not on this list. For completeness: it replaces `xla/service/cpu/small_while_loop_hoisting_pass.{h,cc}` and its registration in `CpuCompiler::RunHloPassesAfterLayoutAssn` in `xla/service/cpu/cpu_compiler.cc`, where it runs after `ParallelTaskAssigner` and copy insertion, immediately before the scheduler-facing passes.
...
2. **Per-kernel search at [M3](#m3-cost-modeled-boundaries-and-large-kernels).** For the few decisions where the static model's error bars are wide and the kernel is hot (tile sizes for large kernels, contested library-vs-native placements), the autotuner searches a space the static model prunes and defaults.

Autotuning cannot do the small-model job itself. That class has thousands of kernels of tens of nanoseconds each, so per-kernel search costs more compile time than it can ever recover, and folding changes which kernels exist while the search runs, so the search target itself is unstable. There the static model decides, and autotuning only supplies its constants.

### 4.6 What remains a boundary

Infeed/outfeed and collectives suspend execution and stay at thunk granularity. Opaque runtime services (FFI custom calls, host callbacks, sort, fft) stay boundaries by default: segmentation folds around them, and only the optional [M5](#m5-optional-demand-driven-mid-kernel-runtime-calls) folds through them. Large library-claimed ops stay thunks because the cost model places them there, not because they are blockers. Every other op can eventually join a kernel.

## 5. Milestones

Each milestone below states what it changes, how to implement it, and what gets measurably faster when it lands. [M1](#m1-the-xtile-computation-driver) can start immediately against the hoisted calls head already produces. [M5](#m5-optional-demand-driven-mid-kernel-runtime-calls) is optional and may never be needed.
...
*Faster:* small stepping models with rolled loops improve 3-4x ([jax#26145](https://github.com/jax-ml/jax/issues/26145): 1.53 to 0.36 ms, bitwise). Loop-dominated scans improve up to 18x ([discussions#24501](https://github.com/jax-ml/jax/discussions/24501)). Any small `lax.scan`/`while_loop` body drops from ~70-90 ns to ~25 ns of overhead per iteration. User guidance flips to "roll your loops on CPU": on jax 0.4.30 a rolled and an unrolled loop ran at the same speed; at head the unrolled form is 4.5x slower and ~12x slower to compile; with [M0](#m0-generalize-hoisting-from-whiles-to-runs) the rolled form is faster to run and faster to compile at every size. Cost: +14% compile time on transformer-shaped models where per-layer bookkeeping folds (bounded, and excluded under the fast-compile preset); large-tensor models are unaffected, because the byte budget excludes them from folding in the first place.

### M1: the xtile computation driver

Build the driver of [4.3](#43-the-xtile-computation-driver): control flow, multi-output, per-edge materialization, memcpy-aware copies. Then perform two separately gated backend flips, in order of risk:
...
2. Standalone loop-path fusions switch from the MLIR loop emitter to xtile. This is the larger flip (526 of 526 census emissions, [5.1](#51-legacy-path-census-on-the-local-corpus) finding 2), and the [5.1](#51-legacy-path-census-on-the-local-corpus) finding-1 recompute class is its named acceptance test.
...
- **Lowering.** Each member lowers through the existing xtile per-op emitters. Intra-kernel edges become tile tensors that bufferize to stack allocations, the default middle point of [4.1](#41-one-kernel-construct-fusion--hoisted-call-materialization-per-edge). This is also the structural fix for the recompute pathology of [5.1](#51-legacy-path-census-on-the-local-corpus) finding 1: a shared producer computes once per tile instead of once per use.
...
- **ABI mapping.** A tuple root maps to multiple result buffers in the kernel ABI; `KernelApiIrBuilder::GetKernelResultsParameters` already emits one result buffer per tuple leaf, so no ABI change is needed. Nonscalar constants lift to operands, a mechanic proven in the prototype. Copies lower to `llvm.memcpy`.
...
- **Where the code lands.** Each op lands with its originating issue's HLO as a regression test plus a latency gate against the thunk path. Analysis-side pieces land in the shared `xla/codegen/tiling/` code ([4.3](#43-the-xtile-computation-driver)).

*Faster:* [jax#37465](https://github.com/jax-ml/jax/issues/37465)-class programs improve ~1.5x (the four scalar reduce-window kernels, ~12 us of ~30 us, become vectorized). [jax#26021](https://github.com/jax-ml/jax/issues/26021)-class dynamic-update-slice-heavy models improve (its 3.7x regression is DUS-dominated; the DUS work item plus [M3](#m3-cost-modeled-boundaries-and-large-kernels) sizing target it). Scatter-heavy loops gain the last ~1.5x if in-kernel scatter ships (0.35 to ~0.24 ms on [jax#26145](https://github.com/jax-ml/jax/issues/26145)).

### M3: cost-modeled boundaries and large kernels

The quantitative model replaces the [M0](#m0-generalize-hoisting-from-whiles-to-runs) gates. Fusion sizing and call hoisting unify under it, per-edge materialization activates, and parallel tiles let big kernels keep the thread pool. The module gate and the protection lists are deleted.

*Implementation:*

- **Machine description.** A struct of dispatch and ABI constants, register file, cache sizes, prefetch stream budget, and pool width, produced by a calibration binary built on the autotuner framework ([4.5](#45-the-cost-model)) and checked in per platform class.
- **One cost library, three clients.** The boundary former (which replaces the [M0](#m0-generalize-hoisting-from-whiles-to-runs) byte budget, member cap, and minimum size with model queries), per-edge materialization inside the [M1](#m1-the-xtile-computation-driver) driver, and library-vs-native placement.
...
- **Shadow mode first.** The model ships log-only against the [M0](#m0-generalize-hoisting-from-whiles-to-runs) heuristics, recording every disagreement on the anchor suite and in production telemetry. The three clients then flip one at a time (boundary former, then per-edge materialization, then library placement). An [M0](#m0-generalize-hoisting-from-whiles-to-runs) gate is deleted only after its replacement client has survived a release.
- **Anchor suite.** The issue repros of [section 6](#6-issue-map) plus a transformer anchor run continuously with bitwise, latency, and compile-time assertions.

*Faster:* medium models: per-layer chains get kernels sized by the model, instead of kernels shaped by whichever fixed threshold happened to bind first. Trace-time-unrolled code (the [jax#26021](https://github.com/jax-ml/jax/issues/26021) shape) folds safely under the compile rails, measured 1.3-1.5x on synthetic unrolled chains. Large-tensor fusions grow without serializing; today's byte gates forbid exactly the folds that would pay there. This is also where compile-time regressions become structurally impossible rather than gated: the budget is a model input.

### M4: delete the legacy emitters

Deletion is inventory-driven. By this point [M1](#m1-the-xtile-computation-driver) has already replaced the hoisted-call backend and [M2](#m2-op-coverage-on-the-modern-path) the reduce-window fallback. The `use_fusion_emitters=false` mode retires with a deprecation window. Then `IrEmitter`, `IrEmitter2`, and `ComputationKernelEmitter` are removed, and the pipeline's dual paths (for example, the unconditional scatter-expansion branch) collapse.
...
- **Work items.** Each surviving consumer gets a work item in [M1](#m1-the-xtile-computation-driver) or [M2](#m2-op-coverage-on-the-modern-path).
...
- **Which corpus authorizes what.** The corpus that authorizes a hard-error flip is the production-derived one described at the end of [5.1](#51-legacy-path-census-on-the-local-corpus), not the local census. The local census ([5.1](#51-legacy-path-census-on-the-local-corpus)) orders the work.

*Faster:* nothing directly. The payoff is maintenance, single-path testing, and the ability to change the kernel ABI ([M3](#m3-cost-modeled-boundaries-and-large-kernels)'s scratch buffers, [M5](#m5-optional-demand-driven-mid-kernel-runtime-calls)'s optional context) without double implementation.

### M5 (optional, demand-driven): mid-kernel runtime calls

Build the runtime-context ABI of [4.2](#42-library-and-runtime-calls-thunks-by-design-native-emission-inside-kernels): context parameter, trampoline registry, FFI call frames. Sort and fft then leave the boundary list. This milestone is built only if a workload appears that folding-around ([M0](#m0-generalize-hoisting-from-whiles-to-runs) segmentation) and HLO expansion cannot satisfy. The reentrancy machinery does not exist until then.
...
- **Landing order, if built.** The ABI change goes first: deleting the duplicate emitters ([M4](#m4-delete-the-legacy-emitters)) is what made ABI changes cheap, and this is the last one planned. Then sort/fft as the low-risk in-tree targets, then FFI frames, then host callbacks.
...
This census measures which emitters actually run on real programs at head, to rank the [M4](#m4-delete-the-legacy-emitters) deletion order and the [M2](#m2-op-coverage-on-the-modern-path) op order. Method: five locally captured programs were recompiled from pre-optimization HLO at head-equivalent defaults, with while hoisting disabled so every op routes individually. Per-emitter counts come from emission logging, amplification from the optimized module structure, and runtimes plus per-thunk attribution from a benchmark harness (Apple M3, single thread).
...
1. **The worst offender is on the modern path, not the legacy path.** mjx kinematics runs 529 ms at head defaults and 0.957 ms with fusions forced to the legacy elemental emitter via `xla_cpu_disable_new_fusion_emitters`: 553x. The hot kernels are 567-571-instruction fusion bodies that produce f32[1,3] outputs in 208 ms and 136 ms per execution. Per-kernel times decay geometrically along the kinematic chain, consistent with per-use recompute turning the shared-producer DAG into a tree (exponential in chain depth); the legacy emitter memoizes elemental values per index. jaxley (1.58 vs 1.52 ms) and mm (0.072 ms both ways) show no gap. A first reduction attempt localizes the defect: a synthetic pure-elementwise chain with two uses per value does not reproduce (LLVM common-subexpression elimination covers it; 0.015 vs 0.020 ms), while the hot bodies share values through slices (single values used five times via slice-of-shared-subexpression, 269 multiplies for a 3-element output), so the recompute survives CSE through differing index expressions. Three consequences follow. First, this is an immediate work item tracked outside this roadmap's milestones: an upstream filing plus a targeted fix in the loop emitter's value cache (`SubgraphConverter::EmitInstruction` in `xla/codegen/emitters/elemental_hlo_to_mlir.cc` already caches values per instruction and index tuple, so the work is finding why slice-fed chains miss that cache), plausibly days of work against [M1](#m1-the-xtile-computation-driver)'s quarters, and the highest return per line in this document. Second, the [M1](#m1-the-xtile-computation-driver) driver's stack-allocation default for intra-kernel edges is the structural fix, and this class is the named acceptance test for the standalone-fusion flip. Third, deleting the legacy fusion path ([M4](#m4-delete-the-legacy-emitters)) is gated on measurements showing this failure class no longer occurs, not on coverage parity alone.
2. **xtile currently emits nothing, and the reason is the op allow-list, not the analysis.** 0 of 526 loop-path fusion emissions were claimed by the tiled emitter, with tiling propagation on or off. Attribution over the 715 loop-path fusion instances: the CPU emitter admits only elementwise ops, bitcast/reshape/transpose/iota, and scalar constants, while the corpus fusions contain slice (279 instances), broadcast (267), dynamic-update-slice (254), concatenate (117), nonscalar constants (95), gather (90), reduce (75), and reverse (52). The shared `SymbolicTileAnalysis` already handles broadcast, reduce, and dot; only the allow-list and per-op lowerings gate them. The rest is emission-side work. About 50 fusions contain only whitelisted ops and still fell back; those failures have not been attributed and deserve investigation before the [M1](#m1-the-xtile-computation-driver) flip. Consequence: the standalone-fusion flip's prerequisite coverage (broadcast, slice, reduce, concatenate, nonscalar constants) is distinct from [M2](#m2-op-coverage-on-the-modern-path)'s issue-driven list and sits inside [M1](#m1-the-xtile-computation-driver)'s second flip, which is why that flip is the larger one.
3. **ElementalKernelEmitter is already unreachable on real programs.** The fusion wrapper turns every surviving standalone elemental op into a single-op loop fusion. The standalone emitter therefore lives only through its fallback conditions (strided copy, non-Eigen convolution, layout-mismatched concatenate, non-in-place DUS), and none of them fired. It is the first hard-error candidate in the [M4](#m4-delete-the-legacy-emitters) protocol.
4. **Amplification is concentrated where [M0](#m0-generalize-hoisting-from-whiles-to-runs) aims.** jaxley executes about 54,000 kernel dispatches per 1.58 ms call (180 amplified thunks times 300 trips), all sub-microsecond kernels. An arithmetic note: 54,000 times the ~50 ns constant of [section 2](#2-background-the-cost-model-of-the-thunk-runtime) exceeds the wall time because dispatches overlap across the executor's thread pool; the constant is a sequential calibration, so it measures the CPU work each dispatch performs; dividing wall time by it does not give a thunk count. diffrax has 90% of its kernels amplified, including its legacy-emitted GEMV dots and its LAPACK calls (the case [M0](#m0-generalize-hoisting-from-whiles-to-runs) folds around, and the anchor workload for [M5](#m5-optional-demand-driven-mid-kernel-runtime-calls)). mjx and mm have zero amplification; their costs are kernel quality (finding 1) and library placement (finding 5) respectively, which [M0](#m0-generalize-hoisting-from-whiles-to-runs) does not touch.
5. **Small-dot placement, measured.** mm makes 40 standalone dot calls per 72 us step. 35 of them are f64[4,4] Eigen library thunks, the exact shape [4.2](#42-library-and-runtime-calls-thunks-by-design-native-emission-inside-kernels) proposes to emit natively once dots sit inside folded kernels.

The census ranks the deletion and coverage work as follows. For [M4](#m4-delete-the-legacy-emitters) deletion order:
...
2. DotKernelEmitter waits for [M1](#m1-the-xtile-computation-driver) native dot emission; its hits are amplified.
3. IrEmitter2's in-place DUS retires with the [M2](#m2-op-coverage-on-the-modern-path) DUS item.
4. IrEmitter2's fusion path goes last: it is currently the working fallback for the finding-1 pathology, and the missing transformer-class measurement lands on it.

For [M2](#m2-op-coverage-on-the-modern-path) op order: dynamic-update-slice first (235 rooted fusions corpus-wide, 27 amplified), gather second (57, 17 amplified), reverse third (8, all amplified). Reduce-window stays issue-driven ([jax#37465](https://github.com/jax-ml/jax/issues/37465) is the anchor; zero corpus instances).

One constraint the document imposes on itself applies here. [Section 4.5](#45-the-cost-model) encodes that thresholds calibrated on partial slices do not transfer, and this census is four programs of one workload class. It is sufficient to order work items; it is not sufficient to flip a fallback to hard-error. Hard-error flips and final deletions are gated on a production-derived corpus (fleet compilation telemetry and the internal benchmark suites, plus modules donated on the tracked issues) confirming zero hits, with the transformer-class gap closed first for anything touching IrEmitter2's fusion path.
...
|---|---|---|
| [jax-ml/jax#26145](https://github.com/jax-ml/jax/issues/26145) (jaxley neuron sim, 4-5x) | thunk count in a 300-iteration loop; scatters fragment foldable runs | [M0](#m0-generalize-hoisting-from-whiles-to-runs) (prototype measured: bitwise, at pre-thunk parity); [M2](#m2-op-coverage-on-the-modern-path) optional scatter adds ~1.5x |
| [jax-ml/jax discussions#24501](https://github.com/jax-ml/jax/discussions/24501) (scan + progress bar) | million-iteration scan next to hoisted invariants; callback splits the loop | [M0](#m0-generalize-hoisting-from-whiles-to-runs) (6-18x, no-callback); callback variant mitigated by [M0](#m0-generalize-hoisting-from-whiles-to-runs) segmentation, fully [M5](#m5-optional-demand-driven-mid-kernel-runtime-calls) |
| [jax-ml/jax#33666](https://github.com/jax-ml/jax/issues/33666) (diffrax stiff, 1.5-2x) | LAPACK FFI + Python callback inside the Newton loop | [M0](#m0-generalize-hoisting-from-whiles-to-runs) partially (runs between the opaque ops fold); fully [M5](#m5-optional-demand-driven-mid-kernel-runtime-calls), or an HLO LU expansion at small sizes |
| [jax-ml/jax#37465](https://github.com/jax-ml/jax/issues/37465) (jnp.diff, 1.7-3.2x) | scalar legacy reduce-window emission | interim HLO rewrite if its windows are non-overlapping (unverified against the issue's HLO); permanently [M2](#m2-op-coverage-on-the-modern-path) |
| [jax-ml/jax#26021](https://github.com/jax-ml/jax/issues/26021) (MJX mass matrix, 3.7x) | trace-time-unrolled joint loops, DUS-heavy, medium tensors | [M2](#m2-op-coverage-on-the-modern-path) (DUS) + [M3](#m3-cost-modeled-boundaries-and-large-kernels) (sizing); library-side batching is a separate upstream conversation |

## 7. Risks

- **[M1](#m1-the-xtile-computation-driver) parity.** Risk: the legacy emitter is battle-tested on control flow, and the xtile driver replays that maturation. Mitigation: the parity harness and regression corpus already exist; keep the legacy fallback flag one full release after parity.
- **Modern-path inversions.** Risk: the census found a 553x case where the current modern emitter loses to the legacy one ([5.1](#51-legacy-path-census-on-the-local-corpus) finding 1), so "modern" cannot be assumed faster per class. Mitigation: latency gates are per program and measured, never inferred from which path is newer; the legacy fusion path deletes last.
...
- **Coverage scope creep ([M2](#m2-op-coverage-on-the-modern-path)).** Risk: op coverage grows without bound. Mitigation: op work is issue-driven with per-op measurement gates. An earlier coverage plan on this project was cancelled by measurement after one week of scoping, and its findings are reused here rather than relitigated.
- **Parallel tiles vs executor parallelism ([M3](#m3-cost-modeled-boundaries-and-large-kernels)).** Risk: one kernel becomes both internally parallel and concurrently overlapped, oversubscribing the pool. Mitigation: the cost model owns the either/or choice per kernel.
- **[M5](#m5-optional-demand-driven-mid-kernel-runtime-calls) reentrancy (if built).** Risk: library calls from kernels can nest pools or deadlock. Mitigation: sequential-context default, explicit pool handoff via the context, a nested-scan stress test, and the [jax#33666](https://github.com/jax-ml/jax/issues/33666) repro as the anchor. The demand gate on [M5](#m5-optional-demand-driven-mid-kernel-runtime-calls) is itself the primary mitigation.
...
- **Hoisted call:** a run outlined into a call and compiled as one kernel. `SmallWhileLoopHoistingPass` creates these for small whiles at head; [M0](#m0-generalize-hoisting-from-whiles-to-runs) generalizes them to arbitrary runs; after [M1](#m1-the-xtile-computation-driver) they emit through xtile; after [M3](#m3-cost-modeled-boundaries-and-large-kernels) "hoisted call" and "fusion" are the same construct.
- **Call form:** the kCall-plus-outlined-computation representation of a kernel, as opposed to a fusion instruction wrapping a fused computation. Buffer assignment recurses into called computations but treats fusion bodies as opaque, so a kernel that needs scratch buffers or contains control flow uses the call form. Hoisted calls are always in it.
...
- **Boundary:** an op that cannot join a kernel. Today: runtime-service ops, library-claimed ops, and coverage gaps. End state: execution-suspending ops, plus opaque runtime services unless the optional [M5](#m5-optional-demand-driven-mid-kernel-runtime-calls) lands.
