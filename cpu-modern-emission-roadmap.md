...
- **Segmentation ([M0](#m0-generalize-hoisting-from-whiles-to-runs)).** Folding the runs between opaque ops leaves a loop body that contains one paying a handful of dispatches per iteration instead of one per instruction.
...
6. **Mid-kernel runtime-call emission** ([4.2](#42-library-and-runtime-calls-thunks-by-design-native-emission-inside-kernels)): optional, [M5](#m5-optional-demand-driven-mid-kernel-runtime-calls), demand-driven; only for the opaque residue.
...
Deletion is inventory-driven. The hoisted-call backend died at [M1](#m1-the-xtile-computation-driver) and the reduce-window fallback at [M2](#m2-op-coverage-on-the-modern-path). The `use_fusion_emitters=false` mode retires with a deprecation window. Then `IrEmitter`, `IrEmitter2`, and `ComputationKernelEmitter` are removed, and the pipeline's dual paths (for example, the unconditional scatter-expansion branch) collapse.
...
- **Landing order, if built.** The ABI change first (it is also the last ABI change [M4](#m4-delete-the-legacy-emitters) makes cheap), then sort/fft as the low-risk in-tree targets, then FFI frames, then host callbacks.
...
4. **Amplification is concentrated where [M0](#m0-generalize-hoisting-from-whiles-to-runs) aims.** jaxley executes about 54,000 kernel dispatches per 1.58 ms call (180 amplified thunks times 300 trips), all sub-microsecond kernels. An arithmetic note: 54,000 times the ~50 ns constant of [section 2](#2-background-the-cost-model-of-the-thunk-runtime) exceeds the wall time because dispatches overlap across the executor's thread pool; the constant is a sequential calibration, so it prices the CPU work of dispatch and is not a divisor of wall time here. diffrax has 90% of its kernels amplified, including its legacy-emitted GEMV dots and its LAPACK calls (the [M0](#m0-generalize-hoisting-from-whiles-to-runs) fold-around case, [M5](#m5-optional-demand-driven-mid-kernel-runtime-calls)'s anchor). mjx and mm have zero amplification; their costs are kernel quality (finding 1) and library placement (finding 5) respectively, which [M0](#m0-generalize-hoisting-from-whiles-to-runs) does not touch.
