...
**Cost gates.** A region is hoisted only if it has at least 4 members or contains control flow, and the byte budget `max(xla_cpu_small_while_loop_byte_threshold, 64KB)` is enforced during partitioning (next paragraph). Setting the option to 0 disables the pass entirely; this sentinel is the A/B and escape hatch used for every baseline below. The byte budget keeps large models untouched: big fusions exceed it and continue to run thunk-granular, preserving inter-kernel parallelism.

**Budgeted segmentation with loop-amplification gating.** The byte budget is not an accept/reject gate on maximal runs; that starves feasible sub-runs. The real case (jax-ml/jax discussion 24501): a cheap million-iteration accumulation loop is scheduled next to the large loop-invariant fusions LICM hoisted out of it, the aggregate blows the budget, the whole run is dropped, and the loop keeps paying per-iteration dispatch (the body-partitioning fallback then wraps the loop body in a kernel and makes it a net regression, 19 to 27 ms). Instead, a run that fits the budget becomes one region; an over-budget run is greedily segmented, cutting at members whose own footprint exceeds the budget and wherever the running total would overflow. A segment is kept only where a loop amplifies its dispatch savings: in a computation executed under a while (directly or through nested control flow), or when the segment itself contains control flow. Once-per-call straight-line segments are discarded: they save nanoseconds of dispatch while serializing thunks the executor can overlap (folding them measured -13% runtime and +25% compile on Gemma3 1B during development). Result on the discussion-24501 repro: the loop's segment survives the cut around the invariant fusions and hoists, 6-18x faster, bitwise-identical, with the big fusions left at per-op granularity as call operands.
...
| jax discussion 24501 (1M-iteration scan next to large invariants) | 19-29 ms | 1-6 ms (6-18x, machine-load dependent) | requires the segmentation above; matches stock jaxlib 0.10.1 |
...
- **Salvage pass for starved control flow.** An earlier fix for the discussion-24501 case kept the all-or-nothing gate but, when an over-budget region contained control flow, re-evaluated each control-flow member individually and wrapped qualifying ones in single-instruction regions on a separate salvaged list with duplicated safety checks. It works for that case, but it is a special case of run segmentation: it rescues only control-flow singletons (not their small glue, not small non-control-flow sub-runs), adds a second region-creation path, and leaves the underlying all-or-nothing behavior in place. The budgeted segmentation above subsumes it with less code and one code path, and the amplification rule adds the discrimination the salvage lacked (its singleton regions were safe, but generalizing it to non-control-flow segments would have hit the Gemma3 serialization regression).
...
4. Should non-amplified straight-line folding exist at all? Every measurement of it is zero or negative (the frag microbenchmark: flat; Gemma3 segments: -13%; unrolled chains: the compile blowup that motivates the cap). Restricting folding to loop-amplified contexts and control-flow-containing regions would subsume the straight-line cap (question 1) and remove the compile-time risk class, at the cost of the modest folded-runtime win on trace-time-unrolled code. Needs one measurement pass over the affected benchmarks before deciding.

## Glossary

- **Thunk:** the thunk runtime's unit of execution; roughly one kernel launch (or copy, or library call) per HLO instruction or fusion. A thunk *execution* costs ~50 ns at small sizes: ~18 ns dispatch plus ~30 ns kernel call ABI.
- **Dispatch:** the executor's fixed cost to schedule and launch one thunk (~18 ns), independent of what the kernel does.
- **Kernel ABI overhead:** the fixed cost inside a kernel call: building argument arrays, loading buffer pointers, storing result pointers (~30 ns at these sizes).
- **Region:** a set of adjacent instructions outlined into one `kCall` and emitted as a single kernel. The pass's output unit.
- **Run:** a maximal stretch of consecutive region-eligible instructions in a computation's topological order, split at boundaries. Regions are formed from runs.
- **Boundary / unavailable instruction:** an op that cannot live inside a single kernel because it needs runtime services (custom-call, infeed/outfeed, sort, fft, collectives), checked recursively through called computations. Boundaries split runs.
- **Amplification / loop-amplified context:** a computation executes *amplified* when it runs once per iteration of an enclosing while loop: a while body or condition, or anything reached from one through nested control flow (conditional branches inherit their parent's amplification). Fixed per-execution overheads inside an amplified computation are paid trip-count times per call (300x in jaxley, 1,000,000x in the discussion-24501 scan), so folding decisions that save fixed overhead are worth trip_count times more there. Entry-level code executes once per call: amplification 1. The pass carries this as a boolean on its worklist and uses it to decide where aggressive folding (segment keeping, and in the internal branch, admitting emitter-protected fusions) pays.
- **Byte budget:** `max(xla_cpu_small_while_loop_byte_threshold, 64KB)`; the summed `bytes_accessed` a region's members may reach. Keeps large-tensor work at thunk granularity, where intra- and inter-kernel parallelism live.
- **Budgeted segmentation:** when a run exceeds the byte budget, cutting it greedily (at members individually over budget, and wherever the running total would overflow) instead of discarding it. A **segment** is one such piece; segments are kept only where amplified or where they contain control flow.
- **Straight-line:** containing no control-flow ops. Once-per-call straight-line folding saves only nanoseconds and has only ever measured flat-to-negative; loop-containing or amplified folding is where the wins are.
- **Worthiness gate:** a region must have enough thunk-generating members to beat its own ABI cost (internal: >= 3, counting only instructions that would emit thunks), or contain control flow.
- **`xla_cpu_small_call`:** the frontend attribute on the outlined `kCall` that tells the thunk emitter to compile the whole callee as one kernel via `ComputationKernelEmitter`.
- **Legacy emitter (`ComputationKernelEmitter` / `IrEmitter`):** the pre-thunk whole-computation LLVM emitter, reused as the region backend; handles control flow natively, one function per region.
- **Sentinel:** setting the threshold option to 0, which disables the pass (and the scatter expander) entirely; the upstream-equivalent baseline in every A/B here.
- **Scatter expansion:** the companion pass (`SmallScatterExpander`) that rewrites small scatters into while+gather+dynamic-update-slice loops so they stop being region boundaries.
- **Fold:** shorthand for "absorb into a region," i.e. stop being a separately dispatched thunk.
- **Inter-thunk overlap:** the executor runs dataflow-independent thunks concurrently on the thread pool; folding independent thunks into one kernel serializes them, which is why non-amplified straight-line segments are discarded (measured -13% on Gemma3 1B when they were not).
- **Partial-model slice:** a benchmark HLO extracted from a larger program. Per-module properties measured on slices (instruction count, total compile time) do not transfer to the full production module; per-run properties do.
