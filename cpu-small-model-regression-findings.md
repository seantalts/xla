...
| **#26145** | jaxley branched-cable sim | 2.5–4.2× | per-iteration **kernel granularity** in while-loop (~28 kernels/step): mostly **memory round-trips (layer 2)**, ~10% dispatch | **PARTIAL FIX LANDED** — body-internal region hoisting (split at scatter); rest needs Stage-1 | — | **1.33× landed**, ~3× remains for Stage-1 |
...
- **Unblock paths:** (a) **Stage-1** — back region emission with the MLIR fusion-emitter stack, which already emits scatter (the design doc's staging anticipated exactly this: Stage-0/legacy `IrEmitter::HandleScatter` is `Unimplemented`, `ir_emitter.cc:1896`; Stage-1/MLIR handles it). (b) **Body-internal maximal regions split at scatter boundaries** — keep scatters as their own thunks, collapse the non-scatter runs between them via the existing `ComputationKernelEmitter`.
- **PARTIAL FIX LANDED (path b).** Verified jaxley's body has 167 kernels / 31 scatter-wrapping boundaries, with non-scatter runs covering 114 of 167 kernels (68%) — long collapsible runs (17, 20, 14, 17, …). Extended `SmallRegionHoistingPass` to partition *inside* while/conditional bodies and committed (`c710a3a39d` on `feat/cpu-small-region-hoisting`). **Measured: jaxley 1.525 → 1.147 ms (1.33×), 0→29 small_calls, output bitwise-identical**, fires at the default 64KB gate. Hardened: a token-threading test (infeed/outfeed in a loop body stay un-hoisted, module re-verifies) and a control-dependency guard (regions with control deps crossing the boundary are skipped, not silently dropped); 14/14 tests pass. **This recovers ~38% of the regression gap (1525→1147 vs old 373µs); the remaining ~3× — collapsing *across* the scatters — needs Stage-1 (scatter-capable MLIR region emission).**
...
`SmallRegionHoistingPass` (`xla/service/cpu/small_region_hoisting_pass.{h,cc}` + test, branch `feat/cpu-small-region-hoisting`, committed `c710a3a39d`) generalizes the while-only hoister to maximal region partitioning with liveness outlining, **including inside while/conditional bodies**, and is wired into the CPU pipeline (replacing `SmallWhileLoopHoistingPass`). 14/14 tests GREEN incl. bug-shaped guards, a token-threading test, and a control-dependency guard. Its `ComputationKernelEmitter` substrate can't emit scatter, so it collapses the non-scatter runs *around* scatters/boundaries rather than across them — which is what delivers jaxley's 1.33×. For straight-line small models (frag-class) it fires but those aren't dispatch-bound, so no win there; the value is the loop-structured case. Full collapse (across scatters) needs the same pass on a **Stage-1 MLIR substrate**.
...
2. **#26145 jaxley — partial fix LANDED (1.33×, committed `c710a3a39d`)**; for the remaining ~3× pursue **Stage-1 region emission** on the MLIR fusion-emitter stack (handles scatter → collapse across scatter boundaries, not just around them). Helps the loop-structured small-model class broadly.
