...
## UPDATE 2, 2026-06-23 — multi-shape driver research: the driver ALREADY EXISTS (xtile `EmitTiledComputation`); approach pivots to region-as-fusion, and Increment-1 work shrinks substantially

Researched the "multi-shape region driver" (Option B as posed above). Key result: **Option B as framed — broaden the partitioner ABI + hand-roll a loop driver — is the WRONG model. A composable tensor-threading driver already exists and is CPU-wired: the xtile tiled emitter.** The plan pivots to **region-as-fusion → route through the existing xtile tiled-fusion path** (the earlier "Option A"), and the genuinely-new code is much smaller than thought.

### Two emission models exist; the elemental one is the dead end, the xtile one is the answer
- **Model 1 (elemental / loop-fusion) — the wall the scaffold hit.** Scalar-per-output-element: `LoopFusionKernelEmitter::EmitEntryFunction` (`loop_kernel_emitter.cc:144-241`) emits one `scf.forall` and per work-item calls the root subgraph func, which returns **scalars**; `ProvideParameter` (`elemental_hlo_to_mlir.cc:1265-1310`) always re-derives operands from the computation parameters (`take_front(num_parameters)`, :1287); `SubgraphToMlir` returns scalar `Value`s (:1536-1545). Every op shares one output-index domain → multi-shape impossible. **Do NOT broaden `computation_partitioner.cc:497` — it lives in this model.**
- **Model 2 (xtile tiled emitter) — the primitive to use (VERIFIED).** `EmitTiledHloInstruction` (`xla/codegen/xtile/codegen/fusion_emitter.cc:447,887`) returns `absl::StatusOr<TensorValue>` (a `tensor<...>` SSA value) and dispatches dot/reduce/broadcast/pad/concat/elementwise/reshape/transpose — the whole frag op set. `EmitTiledComputation` (`fusion_emitter.cc:1034-1069`) IS the schedule-walk driver: it iterates `tiled_computation.instructions()` in order, emits each via `EmitTiledHloInstruction`, and **threads results through `flat_hash_map<const TiledHloInstruction*, TensorValue> values`** (insert at :1060, operands read from prior entries) — differently-shaped intermediates handled natively. CPU already reaches it: `fusion_emitter.cc:284-304` (enable_tiled_emitter) → `tiled_fusion_emitter.cc:305-447 EmitTiledFusionKernel` → `xtile::EmitGeneric` → `EmitTiledComputation`. Empirically, the frag `multiply_tanh` kernel dumps with provenance `xla_cpu_emitter__tiled_emitter` and pre-bufferization IR `xtile.extract → stablehlo.multiply → math.tanh → xtile.insert` over `tensor<1x16xf32>`.

  *Verified independently:* `EmitTiledComputation` at `fusion_emitter.cc:1034` returns `vector<TensorValue>`, threads the `values` map (`:1038,1060`); `EmitTiledHloInstruction` returns `StatusOr<TensorValue>` (`:447,887`).

### Intermediate→alloca: CONFIRMED automatic (static + empirical)
`fusion_compiler.cc:312 AddBufferizationPasses`: OneShotBufferize (:315) → BufferHoisting (:318) → PromoteBuffersToStack `maxAllocSizeInBytes=4096` (:332-335). frag intermediates (4B/64B/1024B) are under threshold; the `multiply_tanh` tiled kernel dumps show `memref.alloca() : memref<1x16xf32>` → `llvm.alloca`. No driver work needed for the alloca win, provided the region emits as one tiled kernel.

### What actually needs building (much smaller than a greenfield driver)
1. Have `SmallRegionHoistingPass` emit the region as a **`kFusion`** (so it reaches the fusion/tiled path) rather than only a `kCall`+attr — or build a fusion view in the emitter (riskier). CPU-local, already-ours pass.
2. **Loosen the CPU `IsSupportedInstruction` gate** (`tiled_fusion_emitter.cc:177`, currently `default: IsElementwise()` → excludes dot/reduce/broadcast — VERIFIED) to admit dot/reduce/broadcast behind the experimental flag. CPU-local, low blast radius.
3. Route `xla_cpu_small_call` (flag on) to the tiled path; keep `RegionKernelEmitter` scaffold as the entry. NO partitioner-ABI changes, NO hand-rolled loops.

### The one unretired, HIGH risk — gate the build on it
Can CPU `SymbolicTileAnalysis` tile a multi-shape graph (especially **dot + reduce**) as ONE fusion? Today the HLO pipeline *splits* frag into three fusions (`multiply_tanh`, `wrapped_reduce`, `broadcast_subtract`); `EnableExperimentalTiling` is off by default and `GetTiledHloComputation` uses a "single-tile dummy" placeholder (`tiled_fusion_emitter.cc:339-353`, TODO b/511084185). If analysis declines dot+reduce in one fusion, Approach A can fold only tileable sub-regions, not the whole frag. **Mitigation: a targeted `SymbolicTileAnalysis` dump on a frag fusion-view BEFORE building** — if it declines dot+reduce, scope Increment 1' to dot-free multi-shape regions (reduce→broadcast etc.) and treat dot-containing regions as a later increment. Also MED risk: `EmitGeneric`/`IsSupportedTiledFusion` currently require a single array root (`fusion_emitter.cc:1085`, `tiled_fusion_emitter.cc:244`) → multi-output regions (e.g. `call.99`'s 4-tuple) deferred; v1 targets single-array-root regions (frag qualifies).

### Revised increment plan (supersedes UPDATE 1's A/B fork)
- **Increment 1'-gate (do FIRST, ~0.5-1 day):** `SymbolicTileAnalysis` spike on the frag fusion-view to retire the dot+reduce-tiling risk. Decides whether v1 includes dot.
- **Increment 1' (~1 week, contingent on the gate):** hoisting-pass-emits-fusion + loosen CPU gate + route + parity, on a dot-free two-shape region first (reduce→broadcast), then frag.
- **Increment 2 — scatter inside the tiled region** (unblocks jaxley 3×): does the xtile path emit scatter, or only `CpuScatterFusion` standalone? (Re-verify — earlier we confirmed scatter is MLIR-emittable, but via `CpuScatterFusion`, which is a *separate* per-fusion emitter, not necessarily an `EmitTiledHloInstruction` case.) This may itself need a tiled scatter op.
- **Increment 3 — control flow** (`scf.while`/`scf.if`).

### Investigation steps (this update)
Read `region_kernel_emitter.cc` (scaffold + wall comment); read `loop_kernel_emitter.cc:144-241` + `elemental_hlo_to_mlir.cc:1265-1310,1536` (Model 1 = scalar, confirmed dead end); grepped `EmitTiled*` in `xtile/codegen/fusion_emitter.cc` and read `EmitTiledComputation` (:1034) + `EmitTiledHloInstruction` (:447,887) — the threading driver; traced CPU wiring `fusion_emitter.cc:284` → `tiled_fusion_emitter.cc:305`; read CPU `IsSupportedInstruction` (`tiled_fusion_emitter.cc:177-203`, `default: IsElementwise()`); grepped callers of `CreateSubgraphMlirFunction`/`PartitionedComputations` (cross-backend incl. GPU → don't touch); read `fusion_compiler.cc:312-335` bufferization; empirical dumps under `/tmp/fragdump` confirmed tiled provenance + `memref.alloca`. Independently re-verified `EmitTiledComputation`/`EmitTiledHloInstruction` signatures and the CPU gate after the spike.

## UPDATE 3, 2026-06-23 — Risk-1 RETIRED: CPU tiling folds multi-shape (incl. dot) into one kernel; v1 includes dot. All Stage-1 feasibility risks now retired.

The gating question — can CPU `SymbolicTileAnalysis` tile a multi-shape graph (dot+reduce+broadcast) as ONE fusion — is answered **YES** for both the dot-free and dot-containing cases.

**Findings (code + empirical, probe reverted):**
- `SymbolicTileAnalysis` (`xla/codegen/tiling/symbolic_tile_analysis.cc`) is the GPU/Triton-derived analysis; it natively handles `kDot` (:140), reductions, concat, pad, control flow — it is fundamentally multi-shape and structurally rejects none of frag's ops. The restrictive layer is purely the CPU emitter's `IsSupportedInstruction` allow-list (`tiled_fusion_emitter.cc:177`, `default: IsElementwise()`).
- **Empirical probe** (temporarily added `kDot/kReduce/kBroadcast → return true` to `IsSupportedInstruction`, rebuilt bench_hlo, ran single-`kFusion` HLOs with `--run_hlo_passes=false --xla_dump_to`; **reverted after**, worktree confirmed clean):
  - **(a) reduce→broadcast:** tiles as ONE kernel on the DEFAULT path (`SymbolicTileAnalysis`+`GetTiling`), provenance `xla_cpu_emitter__tiled_emitter`, correct result.
  - **(b) dot→reduce→broadcast (frag shape):** on the default path fails with `NOT_FOUND: No tile sizes found for instruction: …dot` — NOT structural: the CPU `GetTiling` helper (`tiled_fusion_emitter.cc:159`) builds a `TileMapping` for the ROOT only, leaving the dot's contracting-dim (a hidden tiling parameter, `tiling_specification.cc:72`) unassigned. Via the experimental tiling-propagation path (`--xla_cpu_experimental_enable_tiling_propagation=true`, the `GetTiledHloComputation`/`TilingSpace` path that assigns ALL tiling-space dims) **(b) tiles as ONE kernel, correct (0.7095), 7 allocas (intermediates localized), `tiled_emitter` provenance**.

**Decision: v1 includes dot.** The substrate is fully proven for the frag region shape. Implementation detail (not a risk): for dot on the default path, the CPU `GetTiling` must populate tile sizes for hidden tiling params (dot contracting dim), not just the root — OR route region-fusions through the experimental tiling-propagation path behind the region flag (simplest). 

**Net: all Stage-1 feasibility risks are retired** — scatter is MLIR-emittable (UPDATE 1/GO), the tensor-threading driver exists (`EmitTiledComputation`, UPDATE 2), intermediates→alloca is automatic (UPDATE 2), and multi-shape incl. dot tiles into one kernel (this update). Remaining work is mechanical, CPU-local, flag-gated: (1) hoisting pass emits the region as a `kFusion`; (2) loosen CPU `IsSupportedInstruction` for dot/reduce/broadcast behind the flag; (3) route region-fusions through the experimental tiling-propagation path (or fix `GetTiling`); (4) parity-test, then add scatter (Inc 2 — re-verify scatter is reachable as an `EmitTiledHloInstruction` case vs only standalone `CpuScatterFusion`) and control flow (Inc 3).

**Investigation steps (this update):** read `symbolic_tile_analysis.cc` (`AnalyzeComputation`/`AnalyzeFromInstruction:1190`, kDot:140); read CPU `tiled_fusion_emitter.cc` `IsSupportedInstruction:177`, `GetTiling:159`, `GetTiledHloComputation:328`, `tiling_specification.cc:72`; found `EnableExperimentalTiling`/the `--xla_cpu_experimental_enable_tiling_propagation` flag; built bench_hlo with a temporary `IsSupportedInstruction` loosening; ran reduce→broadcast and dot→reduce→broadcast single-fusion HLOs on both the default and experimental-propagation paths under `--xla_dump_to=/tmp/probe`; inspected provenance tags, alloca counts, results; reverted the probe and verified `git status` clean.
