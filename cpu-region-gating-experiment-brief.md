...
1. **Module-size gate.** Should the pass skip entire modules above an instruction-count threshold? Known points, counted as total post-optimization instructions including fusion bodies: jax discussion 24501 repro = 72 (big win, 6-18x), jaxley = 4,434 (big win, 4.2x), Gemma3 1B = 20,776 (runtime wash, +14% compile), TORAX ~29k (untested, compile-sensitive). A cut near 10k separates every known winner from every known compile-cost case; 2000 would wrongly exclude jaxley. Open sub-questions: (a) where between 5k and 20k does the win/wash boundary actually sit on a wider corpus; (b) is total instruction count the right metric, or should the gate count only thunk-level instructions (excluding fusion-internal ones), which would make jaxley look far smaller; (c) does a realistic counterexample exist: a large module whose runtime is dominated by one hot small loop that the gate would starve. Construct one (e.g. embed the 24501 scan into a gemma-sized module) and measure.
...
4. **In-tree micro/meso benchmarks:** `xla/backends/cpu/benchmarks/*.cc` contain parameterized HLO templates (elementwise, dot, reductions, gather/scatter, convs, select-and-scatter, topk, and an e2e dir). Extract 10-15 representative HLO instantiations at small and medium sizes (write a script that lifts the R"(...)"" HLO strings and substitutes the shape parameters; pick the sizes the benchmark files themselves use). This is the "benchy" middle of the corpus.
...
Phase A (corpus baseline, no code changes): for each module, interleaved D/S/D/S at `--iters` sized so a run takes 2-10 s; record compile ms, run ms, parity, region count, and classify runtime as win (>5%), wash, or loss, same for compile. Deliver the table sorted by instruction count. The module-gate answer falls out: find the count (both metrics) above which no module wins at runtime, and check the counterexample module specifically; if it wins despite being big, the gate needs the thunk-level metric or should be dropped in favor of per-run logic only.
...
2. Recommended gate set with the evidence: module gate yes/no + threshold + metric; cap value; question-4 verdict.
